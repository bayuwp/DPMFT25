...
<div class="flex items-center justify-center min-h-screen bg-cover bg-center"
    ...
    <div class="w-full max-w-md bg-white/20 backdrop-blur-xl shadow-2xl rounded-2xl p-10 relative animate-fade-in hover:shadow-blue-500/40 transition duration-300">
        ...
        <form method="POST" action="{{ route('ppl-ft.login.submit', $slug) }}" class="space-y-5">
            @csrf
            ...
            <div class="relative">
                <span class="absolute inset-y-0 left-3 flex items-center text-blue-600">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 12H8m8 0l-8-6v12l8-6z" />
                    </svg>
                </span>
                <input type="text"
                       value="{{ $slug }}@ft.unesa.ac.id"
                       disabled
                       class="w-full pl-10 pr-3 py-2 bg-white/30 text-white placeholder-white rounded-lg border border-white focus:ring-2 focus:ring-blue-400 focus:outline-none shadow-sm" />
            ...
            {{-- Button --}}
            <button type="submit"
                    class="w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white py-2 rounded-lg shadow-md transform hover:scale-[1.02] transition duration-300">
                LOGIN
            </button>
        </form>
    </div>
