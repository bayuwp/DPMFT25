...
<nav class="bg-gradient-to-r from-blue-800 to-blue-500 shadow-md fixed w-full z-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16 items-center">
            ...
            <div class="hidden md:flex space-x-6 items-center">
                @foreach($menuItems as $item)
                    <a href="{{ url($item['url']) }}"
                       class="relative text-white hover:text-yellow-300 font-medium transition duration-300 group">
                        {{ $item['label'] }}
                ...
                <div class="relative group">
                    ...
                    <div class="absolute left-0 top-full mt-0 w-40 bg-blue-600 rounded shadow-lg
                        ...
                        @foreach($tentangKamiItems as $item)
                            <a href="{{ url($item['url']) }}"
                               class="block px-4 py-2 text-white hover:text-yellow-300 hover:bg-blue-700">
                                {{ $item['label'] }}
                            </a>
                        @endforeach
                    </div>
            ...
            <div class="md:hidden">
                <button id="hamburger-menu" class="text-white focus:outline-none">
                    <svg xmlns="http://www.w3.org/2000/svg" id="menu-icon" class="w-6 h-6" fill="none"
        ...
        <div id="mobile-menu" class="hidden flex-col bg-blue-500 mt-2 rounded-lg shadow-lg p-3 space-y-2">
            ...
            <div class="relative">
                <button class="w-full text-left flex items-center text-gray-100 hover:text-yellow-300 font-medium transition duration-300"
                ...
                <div id="mobile-dropdown" class="hidden mt-2 bg-blue-400 rounded shadow-lg z-50 w-full">
                    @foreach($tentangKamiItems as $item)
                        <a href="{{ url($item['url']) }}" class="block px-4 py-2 text-white hover:bg-blue-100">
                            {{ $item['label'] }}
                        </a>
                    @endforeach
                </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        // Reset menu ketika resize ke desktop
