...
<div class="bg-white p-6 shadow rounded">
    <h2 class="text-2xl font-bold mb-4 text-purple-700">Komisi 3</h2>
    ...
    <form action="{{ isset($editData) ? route('admin.komisi3.update', $editData->id) : route('admin.komisi3.store') }}"
        ...
        <input type="text" name="nama" placeholder="Nama"
               ...
               class="border p-2 w-full rounded focus:ring focus:ring-yellow-200">
        ...
        @if(isset($editData) && $editData->foto)
            <div class="mt-2">
                <img src="{{ asset('storage/'.$editData->foto) }}" class="h-24 mx-auto rounded shadow">
            </div>
        @endif
        ...
        <div class="flex gap-3">
            ...
            @if(isset($editData))
                <a href="{{ route('admin.komisi3.index') }}"
                ...
                @forelse($data as $item)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="border px-4 py-2 text-center">
                            @if($item->foto)
                                <img src="{{ asset('storage/'.$item->foto) }}" class="h-16 mx-auto rounded shadow">
                            @else
                        ...
                        <td class="border px-4 py-2 text-center space-x-2">
                            {{-- Tombol Edit --}}
                            <a href="{{ route('admin.komisi3.edit', $item->id) }}"
                               class="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded shadow">
                                Edit
                            </a>

                            {{-- Tombol Hapus --}}
                            <form action="{{ route('admin.komisi3.destroy', $item->id) }}" method="POST" class="inline"
